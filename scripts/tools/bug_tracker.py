...
class BugTracker:
    ...
    def add_bug(self, title: str, description: str, category: str,
                priority: str = PRIORITY_MEDIUM, reporter: str = "系统",
                module: str = "", version: str = "", assignee: str = "",
                steps_to_reproduce: str = "", expected_result: str = "",
                actual_result: str = "") -> int:
        """
        添加新Bug
    ...
    def list_bugs(self, status: str = None, priority: str = None, 
                  category: str = None, module: str = None) -> List[Dict]:
        """
        列出Bug（支持筛选）
    ...
    def get_statistics(self) -> Dict:
        ...
        for bug in self.bugs:
            ...
            # 统计修复情况
            if bug["status"] in [self.STATUS_NEW, self.STATUS_IN_PROGRESS, self.STATUS_REOPENED]:
                stats["open_count"] += 1
    ...
    def export_to_csv(self, output_file: str = "bug_report.csv") -> str:
        ...
        with open(output_file, 'w', encoding='utf-8-sig', newline='') as f:
            fieldnames = [
                "ID", "标题", "状态", "优先级", "分类", "模块", "版本",
                "报告人", "指派人", "创建时间", "更新时间", "修复时间",
                "关闭时间", "描述", "复现步骤", "预期结果", "实际结果", "备注数"
            ]
    ...
    def print_report(self):
        ...
        if stats['total'] > 0:
            fix_rate = (stats['fixed_count'] + stats['closed_count']) / stats['total'] * 100
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            category = categories[int(cat_choice) - 1] if cat_choice.isdigit() and 1 <= int(cat_choice) <= 6 else BugTracker.CATEGORY_DATA
            ...
            priority = priorities[int(pri_choice) - 1] if pri_choice.isdigit() and 1 <= int(pri_choice) <= 4 else BugTracker.PRIORITY_MEDIUM
            ...
            for bug in bugs[-20:]:  # 显示最近20条
                print(f"#{bug['id']:<5}{bug['status']:<12}{bug['priority']:<10}{bug['title'][:40]:<42}{bug.get('module', ''):<15}")
