...
def verify_pet():
    ...
    # 2. 检查 JS 文件 MIME 类型
    print("\n📦 检查 JS 文件 MIME 类型...")
    r_js = requests.get("http://127.0.0.1:5000/static/js/pet.js", timeout=10)
    content_type = r_js.headers.get("Content-Type", "")

    is_correct_mime = (
        "application/javascript" in content_type or "text/javascript" in content_type
    )
    ...
    # 3. 检查 CSS 文件
    print("\n🎨 检查 CSS 文件...")
    r_css = requests.get("http://127.0.0.1:5000/static/css/pet.css", timeout=10)
    ...
    for sprite_url in sprite_files:
        r_sprite = requests.get(f"http://127.0.0.1:5000{sprite_url}", timeout=10)
