...
class TestRunner:
    ...
    def run_tests(self, test_path, report_name, extra_args=None):
        ...
        report_path = os.path.join(
            self.reports_dir, f"{report_name}_{self.timestamp}.html"
        )
    ...
    def run_api_tests(self):
        """运行所有 API 测试"""
        print("\n🔧 开始运行 API 测试...")
        return self.run_tests("Test/api_tests/", "api_tests", ["-m", "not playwright"])

    def run_ui_tests(self):
        """运行所有 UI 测试"""
        print("\n🎨 开始运行 UI 测试...")
        return self.run_tests("Test/ui_tests/", "ui_tests", ["-m", "playwright"])

    def run_e2e_tests(self):
        """运行端到端测试"""
        print("\n🔄 开始运行 E2E 集成测试...")
        return self.run_tests("Test/e2e_tests/", "e2e_tests")

    def run_all_tests(self):
        """运行所有测试"""
        print("\n🚀 开始运行全部测试套件...")
        return self.run_tests("Test/", "all_tests")
    ...
    def run_smoke_tests(self):
        ...
        return self.run_tests(
            "Test/", "smoke_tests", ["-m", "p0", "-x"]  # -x 遇到第一个失败就停止
        )

    def generate_summary_report(self, results):
        """生成测试总结报告"""
        summary_path = os.path.join(
            self.reports_dir, f"test_summary_{self.timestamp}.txt"
        )
