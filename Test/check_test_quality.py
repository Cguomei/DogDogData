...
class TestQualityChecker:
    ...
    def generate_report(self):
        ...
        if total_issues == 0:
            ...
            print(
                f"  ❌ 严重问题：{len([i for i in self.issues if i['type'] == 'CRITICAL'])}"
            )
        ...
        # 打印严重问题和错误
        critical_and_errors = [
            i for i in self.issues if i["type"] in ["CRITICAL", "ERROR"]
        ]
    ...
    def save_report(self):
        ...
        report_file = (
            report_dir / f'quality_check_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"总计发现问题：{len(self.issues) + len(self.warnings) + len(self.suggestions)}\n"
            )
            f.write(
                f"  严重问题：{len([i for i in self.issues if i['type'] == 'CRITICAL'])}\n"
            )
            f.write(
                f"  错误：{len([i for i in self.issues if i['type'] == 'ERROR'])}\n"
            )
