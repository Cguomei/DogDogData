...
class TestUserRegistrationFlow:
    ...
    def test_register_duplicate_username(self, client):
        ...
        client.post(
            "/register",
            data={"username": "duplicate_test_user", "password": "testpass123"},
        )

        # 尝试用相同用户名注册
        response = client.post(
            "/register",
            data={"username": "duplicate_test_user", "password": "anotherpass"},
            follow_redirects=True,
        )
...
class TestAuthenticationFlow:
    ...
    def test_login_logout_flow(self, client):
        ...
        assert "登录成功" in login_response.get_data(
            as_text=True
        ) or "首页" in login_response.get_data(as_text=True)
...
class TestChartPagesFlow:
    ...
    def test_chart_page_loads(self, client, chart_url, chart_name):
        ...
        # 如果没有加载成功，记录为性能问题
        if (
            "loading" in response_text.lower()
            and "echarts" not in response_text.lower()
        ):
            report_bug(
...
class TestFoodDashboardFlow:
    ...
    def test_food_dashboard_loads(self, client):
        ...
        # 应该包含统计信息
        assert any(
            keyword in response_text for keyword in ["品牌", "价格", "产地", "统计"]
        )
    ...
    def test_api_invalid_data(self, admin_client):
        ...
        if response.status_code == 201:
            report_bug(
                ...
                priority="High",
                module="API 验证",
                steps_to_reproduce=["POST /api/breeds", "发送包含无效数据的 JSON"],
    ...
    def test_api_missing_required_fields(self, admin_client):
        ...
        if response.status_code == 201:
            report_bug(
                ...
                priority="High",
                module="API 验证",
                steps_to_reproduce=["POST /api/breeds", "不发送 breed_name 字段"],
    ...
    def test_sql_injection_attempt(self, client):
        ...
        if "登录成功" in response_text:
            report_bug(
                ...
                priority="High",
                module="安全性",
                steps_to_reproduce=["在登录表单用户名输入：' OR '1'='1", "提交表单"],
