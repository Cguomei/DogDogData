...
class TestBreedManagement:
    ...
    def test_add_breed_success(self, admin_client, db):
        """添加品种 - 成功场景"""
        result = TestResult('TC-BREED-001', 'test_add_breed_success', '品种管理', 'High')
    ...
    def test_add_breed_duplicate(self, admin_client, db):
        """添加品种 - 重复名称"""
        result = TestResult('TC-BREED-002', 'test_add_breed_duplicate', '品种管理', 'High')
    ...
    def test_add_breed_missing_fields(self, admin_client):
        """添加品种 - 缺少必填字段"""
        result = TestResult('TC-BREED-003', 'test_add_breed_missing', '品种管理', 'Medium')
    ...
    def test_get_breeds_list(self, client, db):
        """获取品种列表"""
        result = TestResult('TC-BREED-004', 'test_get_breeds_list', '品种管理', 'Medium')
    ...
    def test_delete_breed(self, admin_client, db):
        ...
        try:
            # 创建测试品种
            breed = DogBreed(breed_name='临时犬种', avg_life_years=10, size_category='中型', popularity=50)
            db.session.add(breed)
            db.session.commit()
            breed_id = breed.id
    ...
    def test_non_admin_cannot_add_breed(self, logged_in_client):
        ...
        try:
            response = logged_in_client.post('/api/breeds', json={
                'breed_name': '未授权犬种',
                'avg_life_years': 10,
                'size_category': '小型',
                'popularity': 50
    ...
    def test_breed_name_validation(self, admin_client):
        """品种名称验证"""
        result = TestResult('TC-BREED-008', 'test_breed_name_validation', '品种管理', 'Medium')
