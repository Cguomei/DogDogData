...
class TestVirtualPetInitialization:
    ...
    def test_status_bars_exist(self, page: Page):
        ...
        assert status_bars.count() == 2, f"应该有 2 个状态条，实际{status_bars.count()}个"
...
class TestPetInteraction:
    ...
    def test_hover_show_action_buttons(self, page: Page):
        ...
        assert action_buttons.count() == 4, f"应该有 4 个动作按钮，实际{action_buttons.count()}个"
...
class TestPetMouseEvents:
    ...
    def test_mouse_leave_event(self, page: Page):
        ...
        assert not is_visible or bubble.get_attribute("class") and "show" not in bubble.get_attribute("class"), \
            "鼠标移开后对话气泡应该隐藏"
