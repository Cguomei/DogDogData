...
class TestUserAuthentication:
    ...
    def test_register_duplicate_username(self, client, db):
        """用户注册 - 重复用户名"""
        result = TestResult(
            "TC-AUTH-002", "test_register_duplicate", "用户认证", "High"
        )
        ...
        try:
            ...
            response = client.post(
                "/register",
                data={"username": unique_username, "password": "AnotherPass456"},
                follow_redirects=True,
            )
    ...
    def test_login_success(self, client, db):
        ...
        try:
            ...
            response = client.post(
                "/login",
                data={"username": unique_username, "password": "correct_password"},
                follow_redirects=True,
            )
    ...
    @test_case(
        "TC-AUTH-004", priority="High", expected="登录失败，提示用户名或密码错误"
    )
    def test_login_wrong_password(self, client, db):
        """用户登录 - 错误密码"""
        result = TestResult(
            "TC-AUTH-004", "test_login_wrong_password", "用户认证", "High"
        )
        ...
        try:
            ...
            response = client.post(
                "/login",
                data={"username": unique_username, "password": "wrong_password"},
                follow_redirects=True,
            )
    ...
    def test_sql_injection_login(self, client, db):
        """SQL 注入攻击 - 登录接口"""
        result = TestResult(
            "TC-AUTH-007", "test_sql_injection_login", "安全性", "Critical"
        )
        ...
        try:
            ...
            # SQL 注入 payload
            payloads = ["' OR '1'='1", f"{unique_username}' --", "' OR 1=1 --"]
