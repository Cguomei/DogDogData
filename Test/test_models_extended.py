...
class TestUserModel:
    ...
    def test_password_hashing(self, app, db, session):
        """测试密码加密"""
        result = TestResult(
            "TC-MODEL-USER-001", "test_password_hash", "模型 - 用户", "High"
        )
    ...
    def test_username_validation_valid(self, app):
        """测试用户名验证 - 有效输入"""
        result = TestResult(
            "TC-MODEL-USER-002", "test_username_valid", "模型 - 用户", "High"
        )
    ...
    def test_username_validation_invalid(self, app):
        """测试用户名验证 - 无效输入"""
        result = TestResult(
            "TC-MODEL-USER-003", "test_username_invalid", "模型 - 用户", "High"
        )
    ...
    def test_is_admin(self, app, db, session):
        """测试管理员权限判断"""
        result = TestResult(
            "TC-MODEL-USER-004", "test_is_admin", "模型 - 用户", "Medium"
        )
...
class TestDogBreedModel:
    ...
    def test_create_breed(self, app, db, session):
        """创建品种记录"""
        result = TestResult(
            "TC-MODEL-BREED-001", "test_create_breed", "模型 - 品种", "Medium"
        )
    ...
    def test_optional_fields_null(self, app, db, session):
        """测试可选字段为空"""
        result = TestResult(
            "TC-MODEL-BREED-002", "test_breed_null_fields", "模型 - 品种", "Low"
        )
    ...
    def test_breed_repr(self, app, db, session):
        """测试品种的字符串表示"""
        result = TestResult(
            "TC-MODEL-BREED-003", "test_breed_repr", "模型 - 品种", "Medium"
        )
...
class TestDataIntegrity:
    ...
    def test_username_uniqueness(self, app, db, session):
        """测试用户名唯一性"""
        result = TestResult(
            "TC-INTEGRITY-001", "test_username_unique", "数据完整性", "High"
        )
    ...
    def test_breed_name_uniqueness(self, app, db, session):
        """测试品种名唯一性"""
        result = TestResult(
            "TC-INTEGRITY-002", "test_breed_name_unique", "数据完整性", "High"
        )
