...
class TestCoverageAnalyzer:
    ...
    def find_test_files(self):
        ...
        self.test_files = [f for f in self.test_dir.glob('*.py') if pattern.match(f.name)]
    ...
    def analyze_module_coverage(self):
        ...
        for module, files in modules.items():
            module_tests = [tc for tc in self.test_cases 
                          if any(tc['file'] == f for f in files)]
    ...
    def generate_text_report(self, output_file=None):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for module, stats in self.coverage_stats.items():
                ...
                coverage_rate = (weighted_score / max_score * 100) if max_score > 0 else 0
                
                f.write(f"  加权覆盖率：{coverage_rate:.1f}%\n\n")
                total_coverage += coverage_rate
            
            avg_coverage = total_coverage / len(self.coverage_stats) if self.coverage_stats else 0
            f.write("-" * 80 + "\n")
            ...
            for tc in sorted(self.test_cases, key=lambda x: (x['file'], x['case_id'])):
                ...
                f.write(f"  {icon.get(tc['priority'], '⚪')} {tc['case_id']} - {tc['function']} [{tc['priority']}]\n")
