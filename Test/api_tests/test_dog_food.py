...
class TestDogFoodAPI:
    ...
    def test_filter_food_by_price_range(self, authenticated_api_client):
        """TC-FOOD-006: 按价格区间筛选"""
        response = authenticated_api_client.get('/api/food/filter?min_price=100&max_price=300')
    ...
    def test_food_search_empty_result(self, authenticated_api_client):
        ...
        response = authenticated_api_client.get('/api/food/search?brand=不存在的品牌XYZ')
    ...
    def test_food_filter_invalid_price(self, authenticated_api_client):
        ...
        response = authenticated_api_client.get('/api/food/filter?min_price=abc&max_price=xyz')
