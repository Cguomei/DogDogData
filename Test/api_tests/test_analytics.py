...
class TestAnalyticsAPI:
    ...
    def test_track_event_validation(self, client):
        """测试事件记录验证"""
        # 缺少 event_name
        response = client.post("/api/analytics/track", json={"event_category": "click"})
        assert response.status_code == 400

        # 正常记录
        response = client.post(
            "/api/analytics/track", json={"event_name": "test_event"}
        )
        assert response.status_code == 201
    ...
    def test_event_categories(self, client):
        ...
        for category in categories:
            response = client.post(
                "/api/analytics/track",
                json={"event_name": f"test_{category}", "event_category": category},
            )
    ...
    def test_get_events_admin(self, admin_client):
        ...
        for i in range(5):
            admin_client.post(
                "/api/analytics/track",
                json={"event_name": f"test_event_{i}", "event_category": "test"},
            )
    ...
    def test_analytics_requires_admin(self, client, create_test_users):
        ...
        client.post(
            "/auth/login",
            data={"username": "user", "password": "Test@123456"},
    ...
    def test_event_filtering(self, admin_client):
        ...
        admin_client.post(
            "/api/analytics/track",
            json={"event_name": "page_home", "event_category": "page_view"},
        )
        admin_client.post(
            "/api/analytics/track",
            json={"event_name": "click_button", "event_category": "click"},
        )
    ...
    def test_custom_properties(self, client):
        ...
        response = client.post(
            "/api/analytics/track",
            json={"event_name": "file_download", "properties": properties},
        )
    ...
    def test_conversion_funnel(self, admin_client):
        ...
        for event in funnel_events:
            admin_client.post(
                "/api/analytics/track",
                json={"event_name": event, "event_category": "funnel"},
            )
    ...
    def test_session_tracking(self, client):
        ...
        for i in range(3):
            response = client.post(
                "/api/analytics/track",
                json={"event_name": f"session_event_{i}", "session_id": session_id},
            )
            assert response.status_code == 201
    ...
    def test_events_pagination(self, admin_client):
        ...
        for i in range(25):
            admin_client.post(
                "/api/analytics/track", json={"event_name": f"paginated_event_{i}"}
            )
        ...
        print(
            f"✅ 分页查询正常，第1页 {len(data['events'])} 条，总共 {data['pagination']['total']} 条"
        )
    ...
    def test_date_range_filter(self, admin_client):
        ...
        response = admin_client.get(
            f"/api/analytics/events?start_date={start_date.isoformat()}&end_date={end_date.isoformat()}"
        )
        assert response.status_code == 200, f"请求失败: {response.status_code}"
