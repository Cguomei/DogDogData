...
class TestContextConversation:
    ...
    def test_context_reference_understanding(self, authenticated_api_client):
        ...
        assert data2['success'] is True
        # 回答应该与金毛相关
        assert '金毛' in data2['answer'] or '容易' in data2['answer'] or '难' in data2['answer']
...
class TestQuestionClassification:
    ...
    def test_price_query_variations(self, authenticated_api_client):
        ...
        for message in test_cases:
            ...
            assert data['type'] == 'price_query', f"消息 '{message}' 应该被分类为 price_query"
    ...
    def test_breed_info_variations(self, authenticated_api_client):
        ...
        for message in test_cases:
            ...
            data = response.get_json()
            assert data['success'] is True
            assert data['type'] == 'breed_info', f"消息 '{message}' 应该被分类为 breed_info"
    ...
    def test_recommendation_variations(self, authenticated_api_client):
        ...
        for message in test_cases:
            ...
            data = response.get_json()
            assert data['success'] is True
            assert data['type'] == 'recommendation', f"消息 '{message}' 应该被分类为 recommendation"
    ...
    def test_chart_generation_detection(self, authenticated_api_client):
        ...
        for message in test_cases:
            ...
            data = response.get_json()
            assert data['success'] is True
            assert data['type'] == 'chart_generation', f"消息 '{message}' 应该被分类为 chart_generation"
...
class TestAnswerQuality:
    ...
    def test_answer_contains_source_hint(self, authenticated_api_client):
        ...
        # 回答应该包含来源提示
        answer = data['answer']
        has_source = '📚' in answer or '💡' in answer or '知识库' in answer or 'AI模型' in answer
