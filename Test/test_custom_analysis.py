...
Golden,60,3500"""
    ...
    def test_upload_excel_file(self, logged_in_client):
        """上传 Excel 文件"""
        result = TestResult(
            "TC-ANALYSIS-002", "test_upload_excel", "自定义分析", "High"
        )
    ...
    def test_upload_unsupported_format(self, logged_in_client):
        """上传不支持的文件格式"""
        result = TestResult(
            "TC-ANALYSIS-003", "test_unsupported_format", "自定义分析", "Medium"
        )
    ...
    def test_generate_chart(self, logged_in_client):
        """生成自定义图表"""
        result = TestResult(
            "TC-ANALYSIS-004", "test_generate_chart", "自定义分析", "Medium"
        )
    ...
    def test_access_analysis_page(self, client):
        """访问自定义数据分析页面"""
        result = TestResult(
            "TC-ANALYSIS-005", "test_analysis_page", "自定义分析", "Low"
        )
...
class TestDataValidation:
    ...
    def test_csv_column_detection(self, logged_in_client):
        """CSV 列名检测"""
        result = TestResult(
            "TC-VALIDATION-001", "test_csv_columns", "数据验证", "Medium"
        )
...
test2,20,200"""
    ...
    def test_empty_file_handling(self, logged_in_client):
        """上传空文件"""
        result = TestResult(
            "TC-VALIDATION-002", "test_empty_file", "数据验证", "Medium"
        )
