...
def get_greeting():
    ...
    if hour < 6:
        return jsonify({"greeting": "夜深了，早点休息吧~ 🌙", "mood": "sleepy"})
    elif hour < 9:
        return jsonify({"greeting": "早上好！新的一天开始啦~ ☀️", "mood": "happy"})
    elif hour < 12:
        return jsonify({"greeting": "上午好！工作效率如何？ 💪", "mood": "energetic"})
    elif hour < 14:
        return jsonify({"greeting": "中午好！记得休息一下哦~ 🍱", "mood": "relaxed"})
    elif hour < 18:
        return jsonify({"greeting": "下午好！继续加油！ 📊", "mood": "focused"})
    else:
        return jsonify({"greeting": "晚上好！今天辛苦了~ 🌟", "mood": "tired"})
...
def get_data_summary():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "数据加载中...", "error": str(e)})
...
def get_page_tip(page_name):
    ...
    return jsonify({"tip": random.choice(page_tips), "page": page_name})
