...
def update_preferences():
    ...
    try:
        ...
        if 'preferred_breeds' in data:
            preference.preferred_breeds = json.dumps(data['preferred_breeds'], ensure_ascii=False)
        
        if 'preferred_size' in data:
            valid_sizes = ['small', 'medium', 'large', 'all']
            if data['preferred_size'] not in valid_sizes:
                return jsonify({'success': False, 'error': f'无效的体型值，支持: {valid_sizes}'}), 400
        ...
        if 'experience_level' in data:
            ...
            if data['experience_level'] not in valid_levels:
                return jsonify({'success': False, 'error': f'无效的经验等级，支持: {valid_levels}'}), 400
        ...
        if 'gender_preference' in data:
            ...
            if data['gender_preference'] not in valid_genders:
                return jsonify({'success': False, 'error': f'无效的性别偏好，支持: {valid_genders}'}), 400
        ...
        if 'response_style' in data:
            ...
            if data['response_style'] not in valid_styles:
                return jsonify({'success': False, 'error': f'无效的回复风格，支持: {valid_styles}'}), 400
...
def get_personalized_recommendations():
    ...
    try:
        ...
        if preference.preferred_size and preference.preferred_size != 'all':
            ...
            if size_keywords:
                size_condition = " OR ".join([f"size LIKE :size{i}" for i in range(len(size_keywords))])
        ...
        logger.info(f"为用户 {current_user.username} 生成个性化推荐: {len(recommendations)}个")
...
def get_preference_stats():
    ...
    try:
        ...
        stats = {
            ...
            'preference_last_updated': preference.updated_at.strftime('%Y-%m-%d %H:%M:%S') if preference and preference.updated_at else None
        }
