...
def call_local_model(messages: list, temperature: float = 0.7) -> str:
    ...
    try:
        if MODEL_TYPE == "ollama":
            ...
            logger.debug(
                f"Ollama请求payload: {json.dumps(payload, ensure_ascii=False)[:500]}..."
            )
            ...
            logger.debug(
                f"Ollama响应: {json.dumps(result, ensure_ascii=False)[:500]}..."
            )
            ...
            logger.debug(
                f"LM Studio请求payload: {json.dumps(payload, ensure_ascii=False)[:500]}..."
            )
            ...
            result = response.json()
            logger.debug(
                f"LM Studio响应: {json.dumps(result, ensure_ascii=False)[:500]}..."
            )
...
def classify_question(question: str) -> dict:
    ...
    if any(keyword in question_lower for keyword in chart_keywords):
        ...
        return {"type": "chart_generation", "params": {"chart_type": chart_type}}

    # 1. 价格查询
    if any(keyword in question_lower for keyword in ["价格", "多少钱", "价位", "售价"]):
        breed = extract_breed_name(question)
        return {"type": "price_query", "params": {"breed": breed}}

    # 2. 品种信息查询（扩展关键词）
    elif any(
        keyword in question_lower
        for keyword in ["介绍", "特点", "习性", "寿命", "性格", "怎么样", "如何"]
    ):
        breed = extract_breed_name(question)
        return {"type": "breed_info", "params": {"breed": breed}}

    # 3. 推荐问题
    elif any(
        keyword in question_lower for keyword in ["推荐", "适合", "新手", "第一次养"]
    ):
        return {"type": "recommendation", "params": {}}

    # 4. 对比问题
    elif (
        "对比" in question_lower
        or "vs" in question_lower
        or ("和" in question_lower and len(question) < 30)
    ):
        breeds = extract_multiple_breeds(question)
        return {"type": "comparison", "params": {"breeds": breeds}}

    # 5. 默认：通用问答
    else:
        return {"type": "general_qa", "params": {}}
...
def handle_price_query(params: dict, context: list = None) -> str:
    ...
    if context and len(context) > 0:
        # 构建包含上下文的提示
        context_text = "\n".join(
            [f"{msg['role']}: {msg['content']}" for msg in context[-4:]]
        )
        prompt = f"""
        你是一个宠物数据顾问。用户询问价格信息。
...
def auto_learn_from_answer(
    question: str, answer: str, question_type: str, feedback_score: float = None
) -> bool:
    """
...
def extract_keywords_enhanced(
    text: str, question_type: str, max_keywords: int = 5
) -> str:
    """
    增强的关键词提取（针对不同类型优化）
...
def ai_chat():
    ...
    try:
        ...
        if is_guest:
            ...
            logger.info(
                f"[{request_id}] 用户: {current_user.username} (ID: {current_user.id})"
            )
        ...
        if len(user_message) > 1000:
            ...
            return jsonify({"success": False, "error": "消息过长（最多1000字符）"}), 400
        ...
        if data.get("source", "model") == "model":
            try:
                learn_success = auto_learn_from_answer(
                    user_message, answer, question_type
                )
                if learn_success:
        ...
        logger.error(
            f"请求用户: {current_user.username if current_user.is_authenticated else '未登录'} (ID: {current_user.id if current_user.is_authenticated else 'N/A'})"
        )
        logger.error(
            f"请求数据: {json.dumps(data, ensure_ascii=False) if 'data' in locals() and data else 'N/A'}"
        )
        logger.error(f"会话ID: {session_id if 'session_id' in locals() else 'N/A'}")
        logger.error(
            f"问题类型: {question_type if 'question_type' in locals() else 'N/A'}"
        )
        ...
        if "ConnectionError" in error_type or "connection" in error_msg.lower():
            ...
            "Database" in error_type
            or "database" in error_msg.lower()
            or "sql" in error_msg.lower()
        ):
...
def model_status():
    ...
    try:
        ...
        if MODEL_TYPE == "ollama":
            ...
            if response.status_code == 200:
                return jsonify(
                    {"status": "online", "type": MODEL_TYPE, "url": base_url}
                )
        ...
        return jsonify({"status": "error", "type": MODEL_TYPE, "error": str(e)})
...
def manual_learn():
    ...
    try:
        ...
        if success:
            return jsonify({"success": True, "message": "学习成功，知识已加入库"})
        else:
            return (
                jsonify({"success": False, "message": "学习失败，可能已存在类似知识"}),
                400,
            )
...
def get_sessions():
    ...
    try:
        ...
        if is_guest:
            ...
            if not guest_user:
                # 如果不存在，返回空列表
                return jsonify({"success": True, "sessions": [], "is_guest": True})

            # 游客：返回guest用户的会话
            sessions = (
                ChatSession.query.filter_by(user_id=guest_user.id, is_active=True)
            ...
            # 登录用户：返回自己的会话
            sessions = (
                ChatSession.query.filter_by(user_id=current_user.id, is_active=True)
                .order_by(ChatSession.updated_at.desc())
                .limit(50)
                .all()
            )  # 最多返回50个会话
...
def create_session():
    ...
    try:
        ...
        session = ChatSession(user_id=user_id_for_session, title=title)
...
def get_session(session_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "session": session.to_dict(), "is_guest": is_guest}
        )
    except Exception as e:
...
def delete_session(session_id):
    ...
    try:
        ...
        logger.info(
            f"删除会话: ID={session_id}, 用户={username}, 删除消息数={deleted_messages}"
        )
...
def submit_feedback():
    ...
    try:
        ...
        if feedback not in ["like", "dislike"]:
            return (
                jsonify({"success": False, "error": "feedback必须是like或dislike"}),
                400,
            )
        ...
        logger.info(
            f"用户 {current_user.username} 对消息 {message_id} 提交反馈: {feedback}"
        )
    ...
    try:
        ...
        for msg in messages:
            if msg.question_type:
                question_types[msg.question_type] = (
                    question_types.get(msg.question_type, 0) + 1
                )
        ...
        summary = {
            ...
            "last_message_at": (
                session.last_message_at.strftime("%Y-%m-%d %H:%M:%S")
            ...
            "duration_minutes": (
                round(
                    (session.last_message_at - session.created_at).total_seconds() / 60,
                    1,
                )
...
def generate_purchase_recommendation(user_preferences: dict) -> str:
    ...
    experience = user_preferences.get(
        "experience", ""
    ...
    if size:
        ...
        if size_keywords:
            size_condition = " OR ".join(
                [f"pet_level LIKE :size{i}" for i in range(len(size_keywords))]
            )
...
def generate_data_analysis_report(time_range: str = "30days") -> str:
    ...
    try:
        ...
        for row in price_dist:
            percentage = (
                (row.count / total_stats.total_count * 100)
