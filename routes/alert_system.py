...
def create_price_alert():
    ...
    try:
        ...
        if not all([breed_name, target_price, condition]):
            return jsonify({'success': False, 'error': '缺少必填字段：breed_name, target_price, condition'}), 400
        
        # 验证条件
        if condition not in ['above', 'below']:
            return jsonify({'success': False, 'error': 'condition必须是above或below'}), 400
        ...
        logger.info(f"用户 {current_user.username} 创建价格预警: {breed_name} {condition} {target_price}")
...
def create_breed_alert():
    ...
    try:
        ...
        if alert_type not in valid_types:
            return jsonify({'success': False, 'error': f'无效的alert_type，支持: {valid_types}'}), 400
        
        # 创建提醒
        alert = BreedAlert(
            user_id=current_user.id,
            alert_type=alert_type,
            filters=json.dumps(data.get('filters', {}), ensure_ascii=False) if data.get('filters') else None,
            notify_email=data.get('notify_email', True),
            notify_in_app=data.get('notify_in_app', True)
        )
...
@alert_bp.route('/api/alerts/notifications/<int:notification_id>/read', methods=['POST'])
@login_required
...
def check_price_alerts():
    ...
    try:
        ...
        # 获取所有活跃的价格预警
        active_alerts = PriceAlert.query.filter_by(is_active=True, triggered=False).all()
        ...
        for alert in active_alerts:
            ...
            if result and result[0]:
                ...
                if should_trigger:
                    ...
                    logger.info(f"价格预警触发: 用户{alert.user_id}, {alert.breed_name} ¥{current_price}")
