...
def update_breed(id):
    ...
    if existing:
        return APIResponse.error('该品种名称已存在', code=400, error_code='DUPLICATE_ENTRY')
...
def get_profile():
    ...
    profile = UserProfile.query.filter_by(user_id=current_app.config.get('current_user_id', None)).first()
...
def get_favorites():
    ...
    favorites = UserFavorite.query.filter_by(user_id=current_app.config.get('current_user_id', None)).all()
...
def add_favorite(breed_id):
    ...
    if existing:
        return APIResponse.error('已收藏该品种', code=400, error_code='ALREADY_FAVORITED')
