...
async def test_pet_functionality():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # ===== 测试 2: 检查日志系统是否加载 =====
            print("\n📋 测试 2: 检查日志系统")
            logger_exists = await page.evaluate('typeof window.PetLogger !== "undefined"')
            if logger_exists:
                print("✅ 日志系统已加载")
            ...
            if pet_count == 1:
                ...
                test_results.append(f"❌ 测试 4 失败：宠物数量错误（期望 1 只，实际{pet_count}只）")
            ...
            if pet_container:
                ...
                if width <= 150 and height <= 200:
                    ...
                    test_results.append(f"❌ 测试 6 失败：宠物尺寸过大（{width:.0f}x{height:.0f}px）")
            ...
            # 统计初始化成功消息
            init_success_count = sum(1 for log in console_logs if '初始化成功' in log or '已就位' in log)
            ...
            if init_success_count == 1 and len(pets_after_reload) == 1:
                ...
                test_results.append(f"❌ 测试 7 失败：初始化了{init_success_count}次（重复初始化）")
            elif len(pets_after_reload) > 1:
                print(f"❌ 刷新后有{len(pets_after_reload)}只宠物（清理失败）")
                test_results.append(f"❌ 测试 7 失败：刷新后有{len(pets_after_reload)}只宠物")
            else:
                print("⚠️ 未找到初始化成功消息")
...
def save_test_report(results):
    ...
    report_file = f'test_pet_report_{timestamp}.txt'
