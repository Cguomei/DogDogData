...
# 数据相关
DATA_UPLOAD_COUNT = Counter("data_uploads_total", "Total Data Uploads", ["file_type"])

CHART_GENERATION_COUNT = Counter(
    "chart_generations_total", "Total Chart Generations", ["chart_type"]
)

# 反馈相关
FEEDBACK_SUBMISSION_COUNT = Counter(
    "feedback_submissions_total", "Total Feedback Submissions", ["feedback_type"]
)

# 埋点相关
EVENT_TRACKING_COUNT = Counter(
    "event_tracking_total", "Total Event Tracking", ["event_category"]
)

# 系统指标
DB_QUERY_LATENCY = Histogram(
    "database_query_duration_seconds", "Database Query Latency", ["query_type"]
)
